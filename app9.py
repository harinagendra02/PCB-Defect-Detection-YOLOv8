...
if uploaded_files:
    ...
    with cb:
        btn = st.button("🟩 RUN PREDICTION", key="pred_btn")
        # style it after render
        st.markdown("""
        <script>
        (function(){
    ...
    if btn:
        ...
        with st.spinner("Running inference..."):
            for f in uploaded_files:
                ...
                res = model.predict(img, device=device, conf=0.25, verbose=False)[0]
                plotted = res.plot()[:, :, ::-1]
                pred_img = Image.fromarray(plotted)

                rows=[]
                for i, b in enumerate(res.boxes):
                    cls = int(b.cls); conf = float(b.conf); label = res.names[cls]
                    x1,y1,x2,y2 = b.xyxy[0].tolist()
                    w,h = x2-x1, y2-y1; cx,cy = x1 + w/2, y1 + h/2
                    sev = "High" if conf>0.85 else "Medium" if conf>0.6 else "Low"
                    rows.append({
                        "Index": i+1, "Type": label, "Confidence": round(conf,3),
                        "Severity": sev, "Location": f"{int(x1)}, {int(y1)} → {int(x2)}, {int(y2)}",
                        "Size": f"{int(w)}×{int(h)}", "Center": f"{int(cx)}, {int(cy)}"
                    })
                type_counts = pd.Series([r["Type"] for r in rows]).value_counts().to_dict()
                st.session_state["pred_results"].append({
                    "name": f.name, "input_pil": img, "result_pil": pred_img,
                    "defect_rows": rows, "type_counts": type_counts
                })
        st.rerun()
